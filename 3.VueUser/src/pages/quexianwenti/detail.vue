...
<div>
	<div :style='{"border":"1px solid #CBCBCB","width":"80%","padding":"25px","margin":"10px auto","borderRadius":"2px","background":"#fff"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'Ξ'" :style='{"fontSize":"14px","lineHeight":"1","justifyContent":"center","display":"flex"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/quexianwenti"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"padding":"0","margin":"0","background":"#fff","display":"flex","width":"65%","position":"relative","order":"1"}'>
			...
			<div class="info" :style='{"border":"1px solid #654B3C","padding":"0","margin":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%"}'>
				<div class="item" :style='{"padding":"10px","margin":"0","alignItems":"center","background":"#FF5852","display":"flex","width":"100%","justifyContent":"space-between"}'>
                    ...
                    {{detail.wentimingcheng}}
                    </div>
				</div>
				<div class="item" :style='{"width":"50%","padding":"5px 20px","margin":"0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#666","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>问题描述</div>
					<div  :style='{"padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#999","flex":"1","height":"auto"}'>{{detail.wentimiaoshu}}</div>
				</div>
				<div class="item" :style='{"width":"50%","padding":"5px 20px","margin":"0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
				...
				<div class="item" :style='{"width":"50%","padding":"5px 20px","margin":"0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#666","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>提出人</div>
				...
				<div class="btn" :style='{"width":"100%","padding":"10px 0","flexWrap":"wrap","justifyContent":"center","display":"flex"}'>
					<el-button :style='{"border":"1px solid #654b3c","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#654b3c","borderRadius":"4px","background":"#fff","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('quexianwenti','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#654b3c","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('quexianwenti','删除')" @click="delClick">删除</el-button>
					<!-- hasChat 无 -->
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
        init() {
          ...
          this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
            if (res.data.code == 0) {
              ...
              this.title = this.detail.wentimingcheng;
              this.detailBanner = this.detail.wentitupian ? this.detail.wentitupian.split(",") : [];
              this.$forceUpdate();
      ...
      nextClick(page) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/quexianwentiAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此缺陷问题？')
			  .then(_ => {
			    this.$http.post('quexianwenti/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
