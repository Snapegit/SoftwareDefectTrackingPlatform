    ...
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="缺陷类别" prop="quexianleibie">
            <el-input  v-model="ruleForm.quexianleibie" 
                placeholder="缺陷类别" clearable :readonly=" false  ||ro.quexianleibie"></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`quexianleibie/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('quexianleibie/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`quexianleibie/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
