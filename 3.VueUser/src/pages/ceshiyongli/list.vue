...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px 10px","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>用例名称：</div>
        <el-input v-model="formSearch.yonglimingcheng" placeholder="用例名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('ceshiyongli','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 10px 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#ad978a","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/ceshiyongliAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"border":"1px solid #654B3C","minHeight":"800px","padding":"10px 0","margin":"10px 0 0","background":"#fff","flex":"1","width":"calc(100% - 270px)","order":"4"}'>
		...
		<div class="list1 index-pv1" :style='{"width":"100%","padding":"0 10px","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}'>
			<div :style='{"border":"1px solid #654B3C","padding":"10px","margin":"10px 1.5%","overflow":"hidden","background":"#ffff","flexDirection":"column","display":"flex","width":"22%","position":"relative","height":"335px"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<img @click.stop="imgPreView(item.yonglitupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"170px"}' v-if="item.yonglitupian && item.yonglitupian.substr(0,4)=='http'" :src="item.yonglitupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.yonglitupian?item.yonglitupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"170px"}' v-else :src="baseUrl + (item.yonglitupian?item.yonglitupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"0 10px","lineHeight":"20px","fontSize":"14px","color":"#333"}' class="name ">用例名称:{{item.yonglimingcheng}}</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('yonglileibie/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'yonglileibie'
      },
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.yonglimingcheng != '') searchWhere.yonglimingcheng = '%' + this.formSearch.yonglimingcheng + '%';
        if (this.curFenlei != '全部') searchWhere.yonglileibie = this.curFenlei;
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`ceshiyongli/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/ceshiyongliDetail', query: params});
      },
