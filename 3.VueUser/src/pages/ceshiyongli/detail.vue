...
<div>
	<div :style='{"border":"1px solid #CBCBCB","width":"80%","padding":"25px","margin":"10px auto","borderRadius":"2px","background":"#fff"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'Ξ'" :style='{"fontSize":"14px","lineHeight":"1","justifyContent":"center","display":"flex"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/ceshiyongli"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"padding":"0","margin":"0","background":"#fff","display":"flex","width":"65%","position":"relative","order":"1"}'>
			...
			<div class="info" :style='{"border":"1px solid #654B3C","padding":"0","margin":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%"}'>
				<div class="item" :style='{"padding":"10px","margin":"0","alignItems":"center","background":"#FF5852","display":"flex","width":"100%","justifyContent":"space-between"}'>
                    ...
                    {{detail.yonglimingcheng}}
                    </div>
				</div>
				<div class="item" :style='{"width":"50%","padding":"5px 20px","margin":"0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#666","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>用例类别</div>
					<div  :style='{"padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#999","flex":"1","height":"auto"}'>{{detail.yonglileibie}}</div>
				</div>
				<div class="item" :style='{"width":"50%","padding":"5px 20px","margin":"0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#666","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>用例内容</div>
					<div  :style='{"padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#999","flex":"1","height":"auto"}'>{{detail.yonglineirong}}</div>
				</div>
				<div class="item" :style='{"width":"50%","padding":"5px 20px","margin":"0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#666","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>关联项目</div>
					<div  :style='{"padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#999","flex":"1","height":"auto"}'>{{detail.guanlianxiangmu}}</div>
				</div>
				<div class="item" :style='{"width":"50%","padding":"5px 20px","margin":"0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
				...
				<div class="item" :style='{"width":"50%","padding":"5px 20px","margin":"0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#666","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>用例资料</div>
					<el-button :style='{"padding":"0px 10px","margin":"0px","color":"#fff","borderRadius":"20px","textAlign":"center","background":"#654b3c","width":"100px","lineHeight":"40px","fontSize":"14px","textDecoration":"none","height":"40px"}' @click="download(detail.yongliziliao)">点击下载</el-button>
				</div>
				<div class="btn" :style='{"width":"100%","padding":"10px 0","flexWrap":"wrap","justifyContent":"center","display":"flex"}'>
					<el-button :style='{"border":"1px solid #654b3c","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#654b3c","borderRadius":"4px","background":"#fff","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('ceshiyongli','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#654b3c","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('ceshiyongli','删除')" @click="delClick">删除</el-button>
					<!-- hasChat 无 -->
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
        init() {
          ...
          this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
            if (res.data.code == 0) {
              ...
              this.title = this.detail.yonglimingcheng;
              this.detailBanner = this.detail.yonglitupian ? this.detail.yonglitupian.split(",") : [];
              this.$forceUpdate();
      ...
      nextClick(page) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/ceshiyongliAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此测试用例？')
			  .then(_ => {
			    this.$http.post('ceshiyongli/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
