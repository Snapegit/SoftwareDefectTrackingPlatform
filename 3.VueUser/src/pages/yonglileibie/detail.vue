...
<div>
	<div :style='{"border":"1px solid #CBCBCB","width":"80%","padding":"25px","margin":"10px auto","borderRadius":"2px","background":"#fff"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'Ξ'" :style='{"fontSize":"14px","lineHeight":"1","justifyContent":"center","display":"flex"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/yonglileibie"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"padding":"0","margin":"0","background":"#fff","display":"flex","width":"65%","position":"relative","order":"1"}'>
			...
			<div class="info" :style='{"border":"1px solid #654B3C","padding":"0","margin":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%"}'>
				...
				<div class="btn" :style='{"width":"100%","padding":"10px 0","flexWrap":"wrap","justifyContent":"center","display":"flex"}'>
					<el-button :style='{"border":"1px solid #654b3c","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#654b3c","borderRadius":"4px","background":"#fff","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('yonglileibie','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#654b3c","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('yonglileibie','删除')" @click="delClick">删除</el-button>
					<!-- hasChat 无 -->
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      nextClick(page) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/yonglileibieAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此用例类别？')
			  .then(_ => {
			    this.$http.post('yonglileibie/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
