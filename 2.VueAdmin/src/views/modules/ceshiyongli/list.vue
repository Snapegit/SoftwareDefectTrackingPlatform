		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">用例名称</label>
			...
			<div :style='{"width":"100%","padding":"10px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<!-- 无 -->
					<el-table-column :resizable='false' :sortable='false' prop="yonglitupian" width="200" label="用例图片">
						<template slot-scope="scope">
							<div v-if="scope.row.yonglitupian">
								<img v-if="scope.row.yonglitupian.substring(0,4)=='http'" :src="scope.row.yonglitupian.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.yonglitupian.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					<el-table-column :resizable='false' :sortable='false'  
						prop="guanlianxiangmu"
						label="关联项目">
						<template slot-scope="scope">
							{{scope.row.guanlianxiangmu}}
						</template>
					</el-table-column>
					<el-table-column :resizable='false' :sortable='false' prop="yongliziliao" label="用例资料">
						<template slot-scope="scope">
							<el-button v-if="scope.row.yongliziliao" type="text" size="small" @click="download(scope.row.yongliziliao)">下载</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='false' :sortable='false'  
...
import AddOrUpdate from "./add-or-update";
	...
	ceshiyonglistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'ceshiyongli/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
